...
public class CommonShowInfoDialog extends Dialog implements OnClickListener {
	...
	private DialogListenerInfo listener;
	...
	public CommonShowInfoDialog(Context context, DialogListenerInfo listener,
                                int vis, CharSequence content, String strLeft, String strRight) {
		this(context);
		this.context = context;
...
//		setCanceledOnTouchOutside(true);
	...
	public void onClick(View v) {
		if (btn_dialog_left == v) {
			if (listener != null) {
				listener.refreshUI(DialogListenerInfo.BTN_LEFT);
			}
		} else if (btn_dialog_right == v) {
			if (listener != null) {
				listener.refreshUI(DialogListenerInfo.BTN_RIGHT);
			}
			if(isRightCancle())
				cancel();
			else
			...
			if (listener != null) {
				listener.refreshUI(DialogListenerInfo.BTN_CANCEL);
			}
		}
		cancel();
	}
	...
	public DialogListenerInfo getListener() {
		return listener;
	}

	public void setListener(DialogListenerInfo listener) {
		this.listener = listener;
	}
